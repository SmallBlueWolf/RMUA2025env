...
namespace ego_planner
{
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    do
    {
      ...
      if (flag_first_call || flag_polyInit || flag_force_polynomial /*|| ( start_pt - local_target_pt ).norm() < 1.0*/) // Initial path generated from a min-snap traj by order.
      {
        ...
        {
          gl_traj = one_segment_traj_gen_fixed(start_pt, start_vel, start_acc, local_target_pt, local_target_vel, Eigen::Vector3d::Zero(), time);
        }
        else
        {
          ...
          Eigen::VectorXd t(2);
          t(0) = t(1) = time / 2;
          gl_traj = minSnapTraj_fixed(pos, start_vel, local_target_vel, start_acc, Eigen::Vector3d::Zero(), t);
        }
      ...
      else // Initial path generated from previous trajectory.
      {
        ...
        if (poly_time > ts)
        {
          PolynomialTraj gl_traj = one_segment_traj_gen_fixed(local_data_.position_traj_.evaluateDeBoorT(t),
                                                  ...
                                                  const std::vector<Eigen::Vector3d> &waypoints, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    PolynomialTraj gl_traj;
    if (pos.cols() >= 3)
      gl_traj = minSnapTraj_fixed(pos, start_vel, end_vel, start_acc, end_acc, time);
    else if (pos.cols() == 2)
      gl_traj = one_segment_traj_gen_fixed(start_pos, start_vel, start_acc, pos.col(1), end_vel, end_acc, time(0));
    else
      return false;
                                         ...
                                         const Eigen::Vector3d &end_pos, const Eigen::Vector3d &end_vel, const Eigen::Vector3d &end_acc)
  {
    ...
    /* waypoints=2,只有初始点和终点 */
      ...
      gl_traj = one_segment_traj_gen_fixed(start_pos, start_vel, start_acc, end_pos, end_vel, end_acc, time(0));
    else
      return false;
